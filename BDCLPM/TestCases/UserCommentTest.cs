...
public class UserCommentTest
{
    ...
    public static void RunAllTests(IWebDriver driver)
    {
        ...
        string sheetName = "Integrated TC Binh luan";
        ...
        try
        {
            Login(driver, "user3@test.com", "123456");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"⚠️ Login that bai: {ex.Message} - Van tiep tuc chay test...");
        }

        // Chay cac test case (da dang nhap san)
        RunTestAndSaveResult(driver, "BL_INT_01", () => Test_BL_INT_01_AddComment(driver), sheetName);
        RunTestAndSaveResult(driver, "BL_INT_02", () => Test_BL_INT_02_EditComment(driver), sheetName);
        RunTestAndSaveResult(driver, "BL_INT_03", () => Test_BL_INT_03_DeleteComment(driver), sheetName);
        RunTestAndSaveResult(driver, "BL_INT_04", () => Test_BL_INT_04_CommentValidation(driver), sheetName);
        RunTestAndSaveResult(driver, "BL_INT_05", () => Test_BL_INT_05_CommentDisplayInfo(driver), sheetName);
    ...
    /// </summary>
    public static bool Test_BL_INT_01_AddComment(IWebDriver driver)
    {
    ...
    /// </summary>
    public static bool Test_BL_INT_02_EditComment(IWebDriver driver)
    {
        ...
        try
        {
            ...
            // Dam bao dang o trang Watch co phan binh luan
            NavigateToWatchPage(driver, js);
    ...
    /// </summary>
    public static bool Test_BL_INT_03_DeleteComment(IWebDriver driver)
    {
    ...
    /// </summary>
    public static bool Test_BL_INT_04_CommentValidation(IWebDriver driver)
    {
    ...
    /// </summary>
    public static bool Test_BL_INT_05_CommentDisplayInfo(IWebDriver driver)
    {
    ...
    {
        // Dismiss alert truoc khi navigate
        try { driver.SwitchTo().Alert().Accept(); Thread.Sleep(300); } catch (NoAlertPresentException) { }
    ...
    private static void NavigateToWatchPage(IWebDriver driver, IJavaScriptExecutor js)
    {
        ...
        // Lay href va navigate truc tiep thay vi click (tranh loi JS click khong navigate)
        string? detailUrl = movieLinks[0].GetAttribute("href");
        Console.WriteLine($"     Tim thay {movieLinks.Count} link phim");
        Console.WriteLine($"     Navigate toi: {detailUrl}");
        ...
        {
            SafeClick(driver, movieLinks[0]);
        }
        AcceptAlertIfPresent(driver);
        Thread.Sleep(3000);
        
        Console.WriteLine($"     URL sau khi navigate: {driver.Url}");
        ...
        if (watchBtn != null)
        {
            // Lay href va navigate truc tiep
            string? watchUrl = watchBtn.GetAttribute("href");
            Console.WriteLine($"     Tim thay: {watchBtn.Text} - {watchUrl}");
